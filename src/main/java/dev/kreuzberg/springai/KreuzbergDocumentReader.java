...
public final class KreuzbergDocumentReader implements DocumentReader {
  ...
  private ExtractionResult extract() throws IOException, KreuzbergException {
    ...
    ExtractionConfig config =
        extractionConfig != null ? extractionConfig : ExtractionConfig.builder().build();
  ...
  private String resolveMimeType() {
    ...
    throw new IllegalStateException(
        "Cannot resolve MIME type: no explicit mimeType and resource has no filename");
  }
  ...
  private List<Document> mapChunksToDocuments(
      List<Chunk> chunks, Map<String, Object> baseMetadata) {
    return chunks.stream()
        .map(
            chunk -> {
              ...
              chunkMeta
                  .getHeadingContext()
                  .ifPresent(hc -> metadata.put("heading_context", toJson(hc)));
              return new Document(chunk.getContent(), metadata);
            })
        .toList();
  }

  private List<Document> mapElementsToDocuments(
      List<Element> elements, Map<String, Object> baseMetadata) {
  ...
  private List<Document> mapPagesToDocuments(
      List<PageContent> pages, Map<String, Object> baseMetadata) {
    return pages.stream()
        .map(
            page -> {
              Map<String, Object> metadata = new LinkedHashMap<>(baseMetadata);
  ...
  private Map<String, Object> buildBaseMetadata(ExtractionResult result) {
    ...
    extractionMetadata
        .getAuthors()
        .ifPresent(v -> metadata.put("authors", String.join(", ", v)));
    extractionMetadata
        .getKeywords()
        .ifPresent(v -> metadata.put("keywords", String.join(", ", v)));
    ...
    extractionMetadata
        .getTags()
        .ifPresent(v -> metadata.put("tags", String.join(", ", v)));
    ...
    result
        ...
        .ifPresent(pw -> metadata.put("processing_warnings", toJson(pw)));
